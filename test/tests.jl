...
a = Poly(1.//convert(Vector{BigInt},gamma(1:17)),"x")
PQexp = Pade(a,8,8)
# Approximately equal, since the Padé approximant has more digits
# using BigInt than the built-in expontential has
@test_approx_eq padeval(PQexp,1.0) exp(1.0)
@test_approx_eq padeval(PQexp,-1.0) exp(-1.0)
...
orthogonal_poly_test(n -> Orthogonal.L(n, Poly([0,1//1])), {[1],[1,-1],[2,-4,1],[6,-18,9,-1],[24,-96,72,-16,1],[120,-600,600,-200,25,-1],[720,-4320,5400,-2400,450,-36,1],[5040,-35280,52920,-29400,7350,-882,49,-1],[40320,-322560,564480,-376320,117600,-18816,1568,-64,1]}, [1 1 2 6 24 120 720 5040 40320]//1)

println("Testing the first 4 associated Laguerre polynomials")
for α=Any[1,1.0,2,2.0,2.1]
    orthogonal_poly_test(n -> Orthogonal.Lα(n,α,Poly([0,one(α)])),
