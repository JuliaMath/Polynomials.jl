...
function Base.convert(::Type{P}, q::StandardBasisPolynomial{T′,X′}) where {P <:LaurentPolynomial,T′,X′}
     T = _eltype(P, q)
     X = indeterminate(P, q)
...
# resolve ambiguity
function Base.convert(::Type{P}, q::Polynomial{T, X}) where {T, X, P<:LaurentPolynomial}
